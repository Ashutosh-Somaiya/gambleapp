...
post '/bet' do
  ...
  if number == roll
    ...
    session[:message] = "Congratulations, you won #{5*money} dollars because you chose #{number} and the dice rolled #{roll}"
  else
    ...
    session[:message] = "Oops, you lost #{money} dollars because you chose #{number} and the dice rolled #{roll}"
  end
  redirect '/bet'
end
